...
class PlateReader:
    ...
    def read(self, image_path):
        ...
        def get_valid_plates(ocr_output, min_conf):
            found = []
            for bbox, text, conf in ocr_output:
                norm = self.normalize_plate(text)
                if len(norm) >= 4 and conf >= min_conf:
                    found.append({"plate": norm, "confidence": conf, "bbox": bbox})
            return found

        # -----------------------------------------
        # PASS 1: The Fast Path
        # -----------------------------------------
        high_conf_plates = get_valid_plates(raw_results, min_conf=OCR_CONF_PRIMARY)
        if high_conf_plates:
            return high_conf_plates
        ...
        return get_valid_plates(enhanced_results, min_conf=OCR_CONF_FALLBACK)